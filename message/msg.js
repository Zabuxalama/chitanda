...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["628565337357@s.whatsapp.net", "628565337357@s.whatsapp.net"].includes(sender) ? true : false
		...
		const buttonsDefault = [
			{ callButton: { displayText: `Call Owner!`, phoneNumber: `+628565337357` } },
			{ urlButton: { displayText: `Instagram!`, url : `https://instagram.com/alwiiyy__` } },
		...
		switch(command) {
			...
			case prefix+'help':
			    ...
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync(setting.pathimg) }, templateButtons: buttonsDefault, footer: 'MiawTeam © 2021', mentions: [sender] })
				break
			case prefix+'runtime':
			...
			case prefix+'donate':
			case prefix+'donasi':
			    reply(`──「 MENU DONATE 」──\n\nHi ${pushname} 👋🏻\n\`\`\`GOPAY/DANA : 08565337357\`\`\`\n\`\`\`PULSA : 085751337466 (Indosat)\`\`\`\nTerimakasih untuk kamu yang sudah donasi untuk perkembangan bot ini _^\n──「 THX FOR YOU ! 」──`)
			    break
			case prefix+'owner':
			    for (let x of ownerNumber) {
			      sendContact(from, x.split('@s.whatsapp.net')[0], 'Jangan Chat Kalo Gak Penting', msg)
			    }
			    break
case prefix+'report':
...
- Rp.50.000 - Lima Bulan`
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync('./media/miaw.jpg') }, templateButtons: buttonsDefault, footer: '© Miaw - Bot', mentions: [sender] })
			    break
			case prefix+'cekprem':
